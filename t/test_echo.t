...
$fleetconf->select_agents(
	sub { shift->header('name') eq 'Test-Echo-Agent' }
);
$fleetconf->run_agents;

my ($agent) = $fleetconf->agents;
print $agent->filename eq 'test_echo.agent' ? 'ok' : 'not ok'," 2\n";
print $agent->name eq 'Test-Echo-Agent' ? 'ok' : 'not ok'," 3\n";
print $agent->description eq 'Simple test for ECHO.' ? 'ok' : 'not ok'," 4\n";
